...
#ifndef TASKPOOL_HPP_INCLUDED
#define TASKPOOL_HPP_INCLUDED
...
/**
 @file task_pool.hpp
 @brief Definition of class TaskPool.
...
namespace scheduler
{
    /**
     ...
     - a map mTasks mapping Thread::tid_t's to the next Instruction 
     posted by the corresponding Thread.
     A TaskPool is equiped with a locking mechanism that allows threads 
     to safely operate on its data concurrently.
     @note The TaskPool assumes that threads don't cheat and post under
     false id.
     */
    class TaskPool
    {
    public:
        ...
        /**
         @brief Mytex protecting mTasks, mStatus, mNr_registered, and 
         mModified.
         */
        std::mutex mMutex;

        /**
         @brief Used to conditionally wait until for a relevant change in the
         TaskPool.
         @details A relevant change (i.e. a Thread's became DISABLED or 
         FINISHED, or a new task has been posted, both affecting whether or 
         not all enabled Threads have posted their task).
         */
        ...
        /**
         @brief Registers a thread with Thread::tid tid to the TaskPool by 
         creating an entry for it in mThreads. The Thread is registered with 
        ...
        /**
         @brief Sets mCurrentTask to the task posted by Thread tid and 
         removes the Instruction from mTasks. Returns a copy of the current 
         task.
         */
        ...
        /**
         @brief mMutex-protected read-only access to the status 
         of Thread tid.
         */
        ...
        /**
         ...
         { (tid, (mTasks[tid], mThread[tid.status == ENABLED)) }
         from this TaskPool.
         */
        NextSet nextset_protected();

        /**
         @brief Creates a program_model::State object from this TaskPool and 
    ...
    private:
        ...
        /**
         @brief Datastrucure mapping Thread::tid_t's to the associated 
         Thread's posted next task.
         */
        Tasks mTasks;
        ...
        /**
         @brief Datastructure mapping Thread::tid_t's to the associated 
         Thread.
         */
        Threads mThreads;

        /**
         @brief Datastructure containing the LockObjects operated on by the
         program.
         */
        LockObs mLockObs;
        ...
        /**
         ...
         @details Enabledness of such a task is determined based on its Op op
         and the status of the Object obj it operates on.
         - If op == LOCK, the task is enabled iff !obj.locked;
         - If op == UNLOCK, we assume that tid holds the lock obj and the 
           task is enabled;
         - Otherwise, the task is enabled.
         If obj is not yet registered in mLockObs, an entry is created.
         */
        void post_lock_instruction(
        ...
        /**
         ...
         @details
         - If task.op == LOCK, all threads waiting on obj are set to DISABLED 
           in mThreads.
         - If task.op == UNLOCK, all threads waiting on obj are set to ENABLED 
           in mThreads.
        ...
        /**
         @brief Set the status of all Threads with lock requests on obj to the 
         given one.
         */
        void set_status_of_waiting_on(
    ...
    // #todo Keep a container of Threads that have unknown status to make
    // the implementation more natural.
