...
namespace scheduler
{
    ...
    void Scheduler::run()
    {
        ...
        while (status() == Execution<State>::Status::RUNNING)
        {
            ...
            auto selection = mSelector->select(
                mPool,
                mLocVars->schedule(),
                mLocVars->task_nr()
            );
            ...
            } else { set_status(selection.first); break; }
            mPool.wait_enabled_collected();
        }
        close(E);
    }
    ...
    /**
     Sets tid as the current thread in TaskPool, removing and
     obtaining the current posted task by Thread tid from the
     TaskPool. Removing the task is to guarantee that the
     Scheduler thread blocks on mPool.all_enabled_collected,
     because the thread only updates its status when it yields
     in task_done. Calls Control to grant Thread tid execution
     right.
     */
    ...
    /**
     @note As soon as internal Scheduler status is set to ERROR 
     new threads are not waiting anymore, but we already waiting
     threads have to be "waken-up".
     */
    ...
    void Scheduler::close(Execution<State>& E)
    {
        ...
        if (!runs_controlled()) { mControl.grant_execution_right_all(); }
        // finish execution
        try {
