...
namespace scheduler
{
    ...
    void TaskPool::set_status_protected(
        const Thread::tid_t& tid, const Thread::Status& status)
    {
        // >>>>> mMutex
        std::lock_guard<std::mutex> guard(mMutex);
        set_status(tid, status);
        if (status == Thread::Status::DISABLED ||
    ...
    NextSet TaskPool::nextset_protected()
    {
        ...
        return utils::maps::zip(
            mTasks, mThreads,
            std::function<next_t(Instruction,Thread)>(
                [] (const auto& task, const auto& thread) {
    ...
    std::unique_ptr<State> TaskPool::program_state()
    {
        ...
        for (const auto& entry : mTasks) {
            ...
            N.insert(NextSet::value_type(
                entry.first,
                next_t{ entry.second, enabled }
            ));
            if (enabled) { Enabled.insert(entry.first); }
        }
    ...
    /**
     @note If task is a LOCK Instruction, its status has to be set
     within the same mLockObsMutex block as the request_lock
     statement, because otherwise the enabledness may be changed by
     a task_done by the lock object's owner.
     @see LockObject::request_lock
     */
    void TaskPool::post_lock_instruction(
        const Thread::tid_t& tid,
