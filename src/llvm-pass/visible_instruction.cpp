...
namespace record_replay
{
   ...
   {
      using namespace utils::io;
      PRINTF(text_color("\t\trecord_replay", Color::YELLOW), "is_visible", "", ""); I->dump();
   ...
   {
      using namespace utils::io;
      PRINTF(text_color("\t\t\trecord_replay", Color::YELLOW), "is_visible", "", "");
      ...
      else if (llvm::Instruction* I = llvm::dyn_cast<llvm::Instruction>(operand))
      {
         ...
         if (llvm::GetElementPtrInst* gep = llvm::dyn_cast<llvm::GetElementPtrInst>(I))
         {
            ...
            if (!gep->getPointerOperand()->getType()->isStructTy())
            {
               ...
               return is_visible(op, gep->getPointerOperand(), gep->getOperand(1), gep->getOperand(2));
            }
