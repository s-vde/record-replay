...
#ifndef INSTRUMENTATION_UTILS_HPP_INCLUDED
#define INSTRUMENTATION_UTILS_HPP_INCLUDED
...
/**
 ...
 @brief Declares a set of utility functions for instrumentation of LLVM IR
 modules.
...
namespace instrumentation_utils
{
     ...
     */
    llvm::FunctionType* create_signature(
        llvm::Function* F,
        const ParamVec& NewParams
    );
    
    /**
     @brief Creates a new llvm::Function with given name and F's signature
     extended with parameters in Params. Returns a pointer to the newly 
     created function and a nullptr when a function with given name already 
     exists in llvm::Module M.
    ...
    /**
     @brief Returns a vector containing all CallInst* with given callee 
     in Function* F.
     */
    CallInstVec call_instructions(
        llvm::Function* F,
    ...
    /**
     @brief Creates a new string GlobalVariable initialized with str 
     in module M and returns a pointer to it.
     */
    llvm::Constant* create_global_cstring_const(
        llvm::Module& M,
        const std::string& name,
