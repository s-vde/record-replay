...
namespace instrumentation_utils
{
        ...
        const ParamVec& NewParams)
    {
        ...
        return
            llvm::FunctionType::get(
                F->getReturnType(),                     // ReturnType
                llvm::ArrayRef<llvm::Type*>(Params),    // Parameters
                Type_F->isVarArg()
            );
    }
    ...
    /**
     @brief Creates a global C-string constant with given name and 
     initialized with a constant array corresponding to given string str.
     @details The type of the llvm::GlobalVariable is an array type
     [str.size()+1 x i8].
     */
