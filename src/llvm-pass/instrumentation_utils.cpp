                                                 ...
                                                 const std::string& unmangled_name)
{
   ...
   for (auto& function : function_list)
   {
      ...
      {
         llvm::errs() << "Function " + unmangled_name + " with mangled name " +
                            function.getName().str() + " found in module\n";
         return &function;
      }
   }
...
llvm::FunctionType* create_signature(llvm::Function* F, const ParamVec& NewParams)
{
   ...
   return llvm::FunctionType::get(F->getReturnType(), llvm::ArrayRef<llvm::Type*>(Params),
                                  Type_F->isVarArg());
}

//--------------------------------------------------------------------------------------------------

llvm::Function* create_function(llvm::Module& M, const std::string& name, llvm::Function* F,
                                const ParamVec& NewParams)
{
   if (M.getFunction(name) == NULL)
   {
      llvm::Function* NewF = llvm::cast<llvm::Function>(
         M.getOrInsertFunction(name, create_signature(F, NewParams), F->getAttributes()));
...
void replace_call(llvm::CallInst* call, llvm::Function* callee, llvm::Function* newcallee,
                  ValueVec NewArgs) // by value
{
   ...
   llvm::CallInst* new_call =
      builder.CreateCall(newcallee, llvm::ArrayRef<llvm::Value*>(NewArgs), call->getName());
...
void replace_invoke(llvm::InvokeInst* invoke, llvm::Function* callee, llvm::Function* newcallee,
                    ValueVec NewArgs)
{
   ...
   llvm::InvokeInst* new_invoke =
      builder.CreateInvoke(newcallee, invoke->getNormalDest(), invoke->getUnwindDest(),
                           llvm::ArrayRef<llvm::Value*>(NewArgs), invoke->getName());
